...
namespace Presentation.Service
{
    public class PhysicalDimensionService : IPhysicalDimensionService
    {
        ...
        public async ValueTask<ApiResult<IEnumerable<PhysicalDimension>>> FindPhysicalDimensionByFilter(PhysicalDimensionFilter fltPhysicalDimension, CancellationToken tknCancellation)
        {
            ...
            if (httpMessage.StatusCode == HttpStatusCode.BadRequest)
                return new ApiResult<IEnumerable<PhysicalDimension>>(new ApiError() { Code = httpMessage.StatusCode.ToString(), Description = await httpMessage.Content.ReadAsStringAsync(tknCancellation) });

            if (httpMessage.StatusCode == HttpStatusCode.Unauthorized)
                return new ApiResult<IEnumerable<PhysicalDimension>>(DefaultApiError.Api.NotAuthenticated);

            if (httpMessage.StatusCode == HttpStatusCode.Forbidden)
                return new ApiResult<IEnumerable<PhysicalDimension>>(DefaultApiError.Api.NotAuthorized);

            if (httpMessage.StatusCode != HttpStatusCode.OK)
                return new ApiResult<IEnumerable<PhysicalDimension>>(DefaultApiError.UnexpectedStatusCode);
            ...
            if (rspnPhysicalDimension is null)
                return new ApiResult<IEnumerable<PhysicalDimension>>(DefaultApiError.DeserializationReturnsNull);

            return new ApiResult<IEnumerable<PhysicalDimension>>(rspnPhysicalDimension.PhysicalDimension.ParseToModel());
        }
