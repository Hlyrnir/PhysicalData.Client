...
namespace Presentation.Service
{
    public class TimePeriodService : ITimePeriodService
    {
        ...
        public async ValueTask<ApiResult<IEnumerable<TimePeriod>>> FindTimePeriodByFilter(TimePeriodFilter fltTimePeriod, CancellationToken tknCancellation)
        {
            ...
            if (httpMessage.StatusCode == HttpStatusCode.BadRequest)
                return new ApiResult<IEnumerable<TimePeriod>>(new ApiError() { Code = httpMessage.StatusCode.ToString(), Description = await httpMessage.Content.ReadAsStringAsync(tknCancellation) });

            if (httpMessage.StatusCode == HttpStatusCode.Unauthorized)
                return new ApiResult<IEnumerable<TimePeriod>>(DefaultApiError.Api.NotAuthenticated);

            if (httpMessage.StatusCode == HttpStatusCode.Forbidden)
                return new ApiResult<IEnumerable<TimePeriod>>(DefaultApiError.Api.NotAuthorized);

            if (httpMessage.StatusCode != HttpStatusCode.OK)
                return new ApiResult<IEnumerable<TimePeriod>>(DefaultApiError.UnexpectedStatusCode);
            ...
            if (rspnTimePeriod is null)
                return new ApiResult<IEnumerable<TimePeriod>>(DefaultApiError.DeserializationReturnsNull);

            return new ApiResult<IEnumerable<TimePeriod>>(rspnTimePeriod.TimePeriod.ParseToModel());
        }
